#!/usr/bin/env python3
"""
Claude Code + Codex memory kit updater.
Safely updates the kit portions of your project without touching your project-specific content.

What it updates:
  CLAUDE.md / AGENTS.md   — session commands block only (## Session Commands → ## Auto-Save Rule)
...
Supported instruction-file heading styles:
...
def extract_kit_block(text):
    """
    Extract the kit-managed block from an instruction file:
    from '## Session Commands' to the next '---' separator.
    """
    m_start = re.search(r'^## Session Commands', text, re.MULTILINE)
    if not m_start:
        return None, "Could not find '## Session Commands' in the kit instruction template"

    after_start = text[m_start.start():]
    m_sep = re.search(r'\n---', after_start)
    if not m_sep:
        return None, "Could not find a '---' separator after '## Session Commands' in the kit instruction template"
...
def main():
    ...
    if not instruction_paths:
        print("\nERROR: No CLAUDE.md or AGENTS.md found. Run this from your project root.")
        sys.exit(1)
    ...
    for path in instruction_paths:
        ...
        if detected_heading is None:
            print(f"\nERROR in {path.name}: {fallback_label}")
            sys.exit(1)
        instruction_texts.append((path, project_text))
        if fallback_label is not None:
            fallback_notes.append((path.name, fallback_label))

    # ── Preview ──
    print("\nFiles that will be updated:")
    for path, _ in instruction_texts:
        print(f"  {path.name:<21} — session commands block only (your project config is safe)")
    ...
    print("  Everything below '## What This Project Is' in CLAUDE.md / AGENTS.md")

    for filename, fallback_label in fallback_notes:
        print(f"\n  Note: {filename} uses '{fallback_label}' as the commands block (non-standard heading detected).")
        print("  After this update, that commands section will be headed '## Session Commands'.")
    ...
    # ── Apply ──
    for path, project_text in instruction_texts:
        updated_text, err = apply_to_project(project_text, kit_block)
        if err:
            print(f"\nERROR updating {path.name}: {err}")
            sys.exit(1)
